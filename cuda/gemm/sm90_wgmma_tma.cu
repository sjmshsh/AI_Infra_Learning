...
__global__ void __launch_bounds__(128, 1)
    wgmma_tma_kernel(const T *A, const T *B, TC *C, int M, int N, int K, float alpha, float beta,
                     ...
                     const __grid_constant__ tmaB tma_b)
{
    ...
    constexpr int num_tma_row_a = bM / 8;
    constexpr int num_tma_col_a = bK / 64;
    constexpr int num_tma_row_b = bK / 8;
    constexpr int num_tma_col_b = bN / 64;
...
#pragma unroll
    for (int pipe = 0; pipe < NumPipe; ++pipe)
    {
        ...
        if (warp_idx == 0 && lane_predicate == 1)
        {
            ...
            tma_copy_a(&tma_a, &producer_mbar[pipe], tile_sA, num_tma_row_a, num_tma_col_a, k_tile * bK, x * bM);
            tma_copy_b(&tma_b, &producer_mbar[pipe], tile_sB, num_tma_row_b, num_tma_col_b, y * bN, k_tile * bK);
        }
        ++k_tile;
    }
...
extern "C" void solve(const half *A, const half *B, half *C, int M, int N, int K, float alpha, float beta)
{
    ...
    // WGMMA的执行单位是warpgroup，一个warpgroup = 4个warp = 128个线程，所以这里一个CUDA block正好放一个warpgroup，用来执行后面的wgmma.mma_async
    constexpr int num_threads = 128; // one warpgroup
    int num_blockM = (M + blockM - 1) / blockM;
    int num_blockN = (N + blockN - 1) / blockN;

    // 这三行本质上是在做grid swizzle / tile swizzle
    // 把逻辑上N方向的 tile 每4个分成一组，然后把这个租内编号塞进 blockIdx.x里
    ...
    // 把global memory 数据搬到 shared memory的时候，shard memory里使用128B swizzle布局
...
/**
 * nvcc sm90_wgmma_tma.cu -O3 -arch=sm_90a -lcuda -lcublas -o sm90_wgmma_tma && ./sm90_wgmma_tma
 * cublas time = 0.187863 ms, TFLPOS = 731.591240, mfu = 0.739728
 * wgmma time = 0.191982 ms, TFLPOS = 715.895551, mfu = 0.723858
 */
int main()
{
    float u = 1.0;
