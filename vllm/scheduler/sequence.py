...
class Sequence:
    block_size = 256
    counter = count() # 一个无限的整数迭代器

    def __init__(self, token_ids: list[int], sampling_params = SamplingParams()):
        self.seq_id = next(Sequence.counter) # 每个序列的唯一标识符
        self.status = SequenceStatus.WAITING # 序列的完成状态
        self.token_ids = copy(token_ids) # 存储序列的所有token ID，包括提示词和生成的内容
